...
public class CellPositions {
    ...
    public static CellPositions from(List<CellPosition> positions) {
    ...
    public static CellPositions from(Cell[][] board) {
        ...
        return from(cellPositions);
    }

    public List<CellPosition> extractRandomPositions(int count) {
        List<CellPosition> cellPositions = new ArrayList<>(positions);
        Collections.shuffle(cellPositions); // 요소들 랜덤을 섞는다

        return cellPositions.subList(0, count);
    }

    public List<CellPosition> subtract(List<CellPosition> positionListToSubtract) {
        List<CellPosition> cellAllPositions = new ArrayList<>(positions);

        CellPositions positionsToSubtract = CellPositions.from(positionListToSubtract);

        return cellAllPositions.stream()
