...
public class OutputHandler {
    ...
    public void showPassOrderSummary(StudyCafeSeatPass cafePass){
        showPassOrderSummary(cafePass, null);
    }


    public void showPassOrderSummary(StudyCafeSeatPass cafePass, StudyCafeLockerPass lockerPass) {
        System.out.println();
        System.out.println("이용 내역");
        System.out.println("이용권: " +  display(cafePass));
        if (lockerPass != null) {
            System.out.println("사물함: " + display(lockerPass));
        }

        double discountRate = cafePass.getDiscountRate();
        int discountPrice = (int) (cafePass.getPrice() * discountRate);
        if (discountPrice > 0) {
            System.out.println("이벤트 할인 금액: " + discountPrice + "원");
        }

        int totalPrice = cafePass.getPrice() - discountPrice + (lockerPass != null ? lockerPass.getPrice() : 0);
